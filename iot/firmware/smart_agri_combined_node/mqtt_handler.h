...
void reconnect() {
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str(), MQTT_USER, MQTT_PASSWORD)) {
      ...
      // Subscribe with wildcard for farmId so it works whether the server sends
      // farm/farm_001/... (string ID) or farm/<uuid>/... (DB UUID)
      String cmdTopic = String("farm/+/device/") + DEVICE_ID + "/command";
