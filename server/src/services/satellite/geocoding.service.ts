...
export const reverseGeocode = async (
  ...
  try {
    ...
    const response = await axios.get(
      `${BHUVAN_API_BASE}/village/reverse`,
      {
        params: { lat, lon },
...
export const geocodeVillage = async (
  ...
  try {
    ...
    const response = await axios.get(
      `${BHUVAN_API_BASE}/village/geocode`,
      {
...
export const getLULCStats = async (
  lat: number,
  lon: number,
...
export const determineLandType = async (
  lat: number,
  lon: number
): Promise<string | null> => {
  try {
    ...
    const lulcData = await getLULCStats(lat, lon);
    ...
    const landTypeMapping: Record<string, string> = {
      'agricultural': 'alluvial',
      'forest': 'forest',
      'barren': 'desert',
      'water': null as any,
      'built-up': null as any
    };
